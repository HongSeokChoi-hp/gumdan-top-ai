...
# ==========================================
# 📚 3. 지식 DB 구축 (핵심: 모든 키 전수조사 방어막)
# ==========================================
@st.cache_resource
def load_or_build_vdb():
    index_path = "faiss_index_saved"
    
    # 🔥 [수정됨] 대충 하나 뽑지 않고, 등록된 "모든" 키와 모델을 돌려보며 확실히 작동하는 놈을 찾습니다.
    embeddings = None
    working_key = None
    ...
    for key in API_KEYS:
        for m_name in ["models/text-embedding-004", "models/embedding-001", "embedding-001"]:
            try:
                temp_emb = GoogleGenerativeAIEmbeddings(model=m_name, google_api_key=key)
                temp_emb.embed_query("test") # 찔러보기 테스트
                embeddings = temp_emb
                working_key = key
                break # 성공하면 모델 루프 탈출
            except:
    ...
    if not embeddings:
        return None, "❌ 등록된 모든 API 키가 구글 서버에서 거부되었습니다. (모두 한도 초과이거나 정지된 키입니다)"
    ...
    if not valid_files:
        return None, "🚨 서버에 'guide.pdf' 또는 'manual2.pdf' 파일이 없습니다."
    ...
    if total_pages == 0: 
        return None, "🚨 PDF 파일에서 텍스트를 읽을 수 없습니다."
    ...
    try:
        ...
        for i in range(0, total_chunks, 100):
            ...
            status_text.markdown(f"🧠 **[2/2] 지식 구조화 중: {chunk_percent}% 완료** (안전 모드 가동 중)")
            time.sleep(1.5)
    ...
    if p := st.chat_input("규정이나 지침을 입력하십시오...", key="search_input"):
        ...
        with chat_box1.chat_message("assistant"):
            try:
                ...
                st.error("⚠️ 시스템 한도가 일시적으로 초과되었습니다. 1분 뒤 다시 시도해 주세요.")
    ...
    if st.button("▶️ 새로운 감독관 질문 받기", use_container_width=True):
        ...
        with chat_box2.chat_message("assistant"):
            try:
                ...
                st.error("⚠️ 한도 초과: 잠시 후 다시 시도해 주세요.")
