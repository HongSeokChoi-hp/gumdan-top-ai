...
if not st.session_state.get("authenticated", False):
    ...
    with col2:
        ...
        if pwd == SET_PASSWORD:
            ...
            if pwd: st.error("❌ 보안 코드가 일치하지 않습니다. 다시 확인해주십시오.")
    st.stop()

# ============================================================
# 🔑 2. [엔진] 무적의 API 키 로테이션 및 자가 치유 엔진 (기능 5, 6, 7)
# ============================================================
...
# [기능 7] 모든 라이브러리를 무시하고 구글 정식 v1 주소로만 통신하는 직결 클래스
class UltimatePaidTierEmbeddings:
    def __init__(self, api_key):
        self.api_key = api_key
        # 유료 계정이면 v1beta가 아닌 정식 v1 경로가 안전합니다.
        self.base_url = "https://generativelanguage.googleapis.com/v1/models/text-embedding-004:embedContent"
    ...
    def embed_query(self, text):
        ...
        if response.status_code != 200:
            # 여기서 발생하는 에러가 진짜 "범인"입니다.
            st.error(f"🚨 구글 서버 응답 오류 (코드: {response.status_code})")
            st.code(response.text) # 구글이 보낸 영문 원문을 그대로 출력
            raise Exception("구글 서버 거부")
...
def load_or_build_vdb():
    ...
    # [기능 7] 유료 티어 직결 엔진 가동
    try:
        embeddings = UltimatePaidTierEmbeddings(working_key)
        embeddings.embed_query("connection_test") # 찔러보기
    except Exception:
        return None, "구글 지능형 서버 연결 실패"
