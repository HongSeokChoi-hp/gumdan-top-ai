...
# ============================================================
# 📚 3. [지식] 지능형 데이터 분석 및 404 차단 (기능 7, 8, 9, 10)
# ============================================================
@st.cache_resource
def load_or_build_vdb():
    ...
    # [기능 7] 404 에러 원천 차단: 구글이 원하는 모든 이름 조합을 전수조사합니다.
    embeddings = None
    last_raw_error = ""

    # 유료 티어와 무료 티어, 최신형과 구형 이름을 모두 리스트에 담았습니다.
    model_name_candidates = [
        "text-embedding-004",           # 1. 신형 (v1 주소용)
        "models/text-embedding-004",    # 2. 신형 (풀네임)
        "embedding-001",                # 3. 표준형
    ...
    # [기능 4] 용어 순화: 'PDF'라는 말 대신 '인증 지침서'로 통합 관리
    ...
    try:
        ...
        for i in range(0, total_chunks, 100):
            ...
            status_text.markdown(f"🧠 **지능형 지식 최적화 구축 중: {c_percent}% 완료** (구글 서버 보호 모드 가동)")
            # [기능 10] 구글 서버 과부하 방지용 정밀 휴식 (1.6초)
            time.sleep(1.6) 
            
        # [기능 8] 완성된 지식 뇌를 영구 저장하여 다음부터는 로딩 없이 0초 실행
        vector_db.save_local(index_path)
        progress_bar.empty()
        status_text.empty()
        return vector_db, None
    except Exception as e:
        return None, f"❌ 지식 최적화 구축 중 오류가 발생했습니다: {str(e)}"
...
with tab1:
    ...
    if query := st.chat_input("조사 규정이나 지침에 대해 질문하십시오...", key="search_input"):
        ...
        with chat_box1.chat_message("assistant"):
            try:
                ...
                st.error("⚠️ 시스템 한도가 일시적으로 초과되었습니다. 잠시 후 다시 시도해 주십시오.")
    ...
    if st.button("▶️ 새로운 감독관 현장 질문 생성", use_container_width=True):
        ...
        with chat_box2.chat_message("assistant"):
            try:
                ...
                inspector_prompt = "병원 인증평가 감독관이 직원에게 던질법한 짧고 날카로운 현장 질문 하나만 한국어로 해줘. (예: 화재 발생 시 가장 먼저 취해야 할 행동은?)"
                ...
                st.error("⚠️ 질문을 생성할 수 없습니다. 엔진 상태를 확인하십시오.")
        ...
        if st.session_state.current_q and st.session_state.current_q != "생성중":
            ...
            with chat_box2.chat_message("assistant"):
                try:
                    ...
                    st.session_state.current_q = None # 답변 완료 후 질문 대기 상태로 초기화
                except Exception:
                    st.error("⚠️ 채점 엔진에 오류가 발생했습니다.")
        else:
            st.warning("위쪽의 '질문 생성' 버튼을 먼저 눌러 감독관의 질문을 받아주십시오.")
