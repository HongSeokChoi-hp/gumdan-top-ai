...
# [수정] PC/모바일 반응형 동작을 위해 initial_sidebar_state="auto" 설정
...
<style>
    ...
    * { font-family: 'Pretendard', sans-serif; }
    ...
    /* 전체 배경색 및 기본 글자색 고정 (다크모드 글씨 증발 방지) */
    ...
    .enterprise-header { 
        ...
        padding: 10px 15px; /* 무식하게 컸던 여백 대폭 축소 */
        border-radius: 8px; 
        margin-top: 5px;
        margin-bottom: 5px;
    ...
    /* 헤더 안의 글씨는 무조건 흰색으로 강제 고정 */
    .enterprise-header h1 { 
        margin: 0; 
        font-size: 1.2rem !important; /* 폰트 크기 슬림화 */
        font-weight: 800; 
        color: #ffffff !important; 
    }
    .enterprise-header img { height: 24px !important; } /* 로고 축소 */
...
if os.path.exists("검단탑병원-로고_고화질.png"):
    with open("검단탑병원-로고_고화질.png", "rb") as image_file:
        ...
        logo_html = f"<img src='data:image/png;base64,{encoded_string}' style='height:24px; background-color:white; padding:3px; border-radius:4px;'>"
...
with tab1:
    # 🚨 [핵심 해결] 고정 높이의 채팅 상자를 만들어, 상자 안에서만 스크롤되도록 설정
    # 이렇게 하면 전체 화면이 내려가지 않고 입력창이 하단에 예쁘게 고정됩니다.
    chat_box = st.container(height=550, border=False)
    
    for m in st.session_state.search_msgs:
        with chat_box.chat_message(m["role"]): st.markdown(m["content"])

    # 입력창은 채팅 상자 '바깥(아래)'에 위치하여 카톡처럼 하단에 고정됨
...
with tab2:
    ...
    # 감독관 탭도 동일하게 고정 높이 상자 적용
    chat_box2 = st.container(height=480, border=False)
